...
func TestEvalXOR(t *testing.T) {
	...
	tests := []struct {
		...
		{"XOR(TRUE,FALSE,TRUE)", false}, // 2 trues => even => false
		{"XOR(TRUE,FALSE,FALSE)", true}, // 1 true => odd => true
	}
...
func TestEvalROWCOLUMN(t *testing.T) {
	resolver := &mockResolver{}
	ctx := &EvalContext{CurrentCol: 3, CurrentRow: 7}
	...
	// Without context, should return #VALUE!
	cf = evalCompile(t, "ROW()")
	got, err = Eval(cf, resolver, nil)
	if err != nil {
		t.Fatalf("Eval: %v", err)
	...
	if got.Type != ValueError || got.Err != ErrValVALUE {
		t.Errorf("ROW() without context = %v, want #VALUE!", got)
	}
}
...
func TestEvalROWSCOLUMNS(t *testing.T) {
	resolver := &mockResolver{
		cells: map[CellAddr]Value{
	...
	cf := evalCompile(t, "ROWS(A1:C2)")
	got, err := Eval(cf, resolver, nil)
	if err != nil {
		t.Fatalf("Eval: %v", err)
	}
	if got.Type != ValueNumber || got.Num != 2 {
		t.Errorf("ROWS(A1:C2) = %g, want 2", got.Num)
	}

	cf = evalCompile(t, "COLUMNS(A1:C2)")
	got, err = Eval(cf, resolver, nil)
	if err != nil {
		t.Fatalf("Eval: %v", err)
	}
	if got.Type != ValueNumber || got.Num != 3 {
		t.Errorf("COLUMNS(A1:C2) = %g, want 3", got.Num)
	}
}
