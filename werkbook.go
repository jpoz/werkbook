...
func cellDataToValue(cd ooxml.CellData) Value {
	switch cd.Type {
	case "s":
		// Shared-string cells must remain strings regardless of content.
		// In Excel, comparing a string "5" to the number 5 returns FALSE
		// (different types), so we must preserve the string type.
